...
public class BoggleSolver {
    ...
    public Iterable<String> getAllValidWords(BoggleBoard board) {
        ...
        // adjacent dice 
        ArrayList<Bag<Integer>> adjbag = new ArrayList<Bag<Integer>>(m * n);
        for (int i = 0; i < m * n; i++) {
            adjbag.add(new Bag<Integer>());
        }
        adjacentBag(adjbag, m, n);
       
        ArrayList<Iterator<Integer>> adj = new ArrayList<Iterator<Integer>>(m * n);
        for (int v = 0; v < m * n; v++) {
            adj.add(v, adjbag.get(v).iterator());
        }        
        ...
        for (int s = 0; s < m * n; s++) {
            ...
            while (!stack.isEmpty()) {
                ...
                if (adj.get(v).hasNext()) {
                    ...
                    adj.set(v, adjbag.get(v).iterator());
                }
            }
        }
        return set;
    ...
    private void adjacentBag(ArrayList<Bag<Integer>> bag, int m, int n) {
        if (m == 1 && n == 1) return;
        else if (m == 1) {
            for (int i = 0; i < n; i++) {
                if (i == 0) bag.get(0).add(1);
                else if (i == n - 1) bag.get(i).add(n - 2);
                else {
                    bag.get(i).add(i - 1);
                    bag.get(i).add(i + 1);
                }
            ...
            for (int i = 0; i < m; i++) {
                if (i == 0) bag.get(0).add(1);
                else if (i == m - 1) bag.get(i).add(m - 2);
                else {
                    bag.get(i).add(i - 1);
                    bag.get(i).add(i + 1);
                }
            ...
            for (int i = 1; i < m - 1; i++) {
                for (int j = 1; j < n - 1; j++) {
                    bag.get(i * n + j).add((i - 1) * n + j - 1);
                    bag.get(i * n + j).add((i - 1) * n + j);
                    bag.get(i * n + j).add((i - 1) * n + j + 1);
                    bag.get(i * n + j).add(i * n + j - 1);
                    bag.get(i * n + j).add(i * n + j + 1);
                    bag.get(i * n + j).add((i + 1) * n + j - 1);
                    bag.get(i * n + j).add((i + 1) * n + j);
                    bag.get(i * n + j).add((i + 1) * n + j + 1);
            ...
            // location (0, n - 1)
            bag.get(n - 1).add(n - 2);
            bag.get(n - 1).add(2 * n - 1);
            bag.get(n - 1).add(2 * n - 2);
            // location (m - 1, 0)
            bag.get((m - 1) * n).add((m - 1) * n + 1);
            bag.get((m - 1) * n).add((m - 2) * n);
            bag.get((m - 1) * n).add((m - 2) * n + 1);
            // location (m - 1, n - 1)
            bag.get(m * n - 1).add(m * n - 2);
            bag.get(m * n - 1).add((m - 1) * n - 1);
            bag.get(m * n - 1).add((m - 1) * n - 2);
            // top
            for (int i = 1; i <= n - 2; i++) {
                bag.get(i).add(i - 1);
                bag.get(i).add(i + 1);
            ...
            // bottom
            for (int j = 1; j <= n - 2; j++) {
                bag.get((m - 1) * n + j).add((m - 1) * n + j - 1);
                bag.get((m - 1) * n + j).add((m - 1) * n + j + 1);
                bag.get((m - 1) * n + j).add((m - 2) * n + j - 1);
                bag.get((m - 1) * n + j).add((m - 2) * n + j);
                bag.get((m - 1) * n + j).add((m - 2) * n + j + 1);
            }
            // left
            for (int i = 1; i <= m - 2; i++) {
                bag.get(i * n).add((i - 1) * n);
                bag.get(i * n).add((i - 1) * n + 1);
                bag.get(i * n).add(i * n + 1);
                bag.get(i * n).add((i + 1) * n);
                bag.get(i * n).add((i + 1) * n + 1);
            }
            // right
            for (int i = 1; i <= m - 2; i++) {
                bag.get((i + 1) * n - 1).add(i * n - 1);
                bag.get((i + 1) * n - 1).add(i * n - 2);
                bag.get((i + 1) * n - 1).add((i + 1) * n - 2);
                bag.get((i + 1) * n - 1).add((i + 2) * n - 1);
                bag.get((i + 1) * n - 1).add((i + 2) * n - 2);
            }            
        }
    }
